...
#### To add a new IGES test: ####

The IGES suites (`IgesImport`, `IgesExport`, plus the `[Explicit]` `-future` and `-large`
fixtures) are the STEP suites' pattern applied to a second format: the same sidecar oracles
(`<model>.expected.txt` / `<model>.exported.txt`, only the keys present are asserted), the same
`#` / `!` file name conventions, the same per-fixture `Regenerate` baseline author - on the
`MX_IGES_*` / `MX_IGESEXPORT_*` environment variables and the `models\IGESfile*\` folders, with
model extensions `.igs` / `.iges`.

Two things are IGES-specific and worth knowing before writing a baseline: Rhino's IGES reader
does not join faces, so a solid arrives as one brep per face with `solids 0`; and the structural
checks a written file gets with no baseline validate the fixed-format S/G/D/P/T layout (80-column
records, per-section sequence numbers, Terminate record counts) rather than STEP's Part 21
markers.

The committed corpus is generated by Rhino's own writer from geometry stated in
`FileIO\IGES\AuthorIgesCorpus.cs` - unencumbered and reviewable, at the cost of only exercising
Rhino's own dialect; externally-originated files with clean rights are welcome additions, and
unpublishable customer files go in `private_models\models\IGESfile*\`.
...
#### To add a new STL test: ####

The STL suites run on `MX_STL_*` / `MX_STLEXPORT_*` and `models\STLfile*\`. STL carries
triangles and nothing else, so the export baselines read as a list of what vanished. The
no-baseline structural checks split on the pinned `binaryfile` option: for binary, file length
must equal `84 + 50 x triangle count` exactly; for ASCII, the solid/facet/loop grammar must
balance. Always use `FileStl.Read`/`FileStl.Write` with explicit options - a bare `doc.Import`
takes weld/split/units from the machine's plugin profile.
